...
local LOBBY_POSITION = Vector3.new(14, 505, -51)
...
-- Debug mode
local DEBUG_MODE = false  -- Set to false to disable debug prints
...
local function EnableLayingDown()
    ...
    -- Snap the character flat instantly
...
local function modifyCoin(coin)
    if coin:IsA("BasePart") and not modifiedCoins[coin] then
        ...
        coin.Size = Vector3.new(5, 5.25, 5)
		coin.Transparency = 1
...
local function startAutofarm()
    ...
    local function checkEliteGamepass()
        ...
        if success then
            ...
            coinLimit = hasEliteGamepass and 50 or 40
    ...
    local function getCoinCount()
        ...
        local coinContainer = container:FindFirstChild("Coin")
        if not coinContainer then return 0 end
        
        local currencyFrame = coinContainer:FindFirstChild("CurrencyFrame")
    ...
    local function collectCoin(coin)
        ...
        if distance > INSTANT_TELEPORT_DISTANCE then
            ...
            -- Snap character to flat rotation before tweening so it doesn't slowly rotate during the tween
...
TeleportTab:AddButton({
    Text = "Lobby",
    Callback = function()
	    local targetPosition = Vector3.new(14, 505, -51)
	    local player = Players.LocalPlayer
        local character = player.Character
